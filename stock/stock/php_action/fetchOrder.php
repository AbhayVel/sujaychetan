...
$sql = "SELECT order_id, order_date, client_name, client_contact, payment_status FROM orders WHERE order_status = 1 order by order_date desc";
...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
 	...
 	if($row[4] == 1) { 		
 		...
 		$paymentStatus = "<label class='label label-info' style='background-color:#F0AD01'>Pending Payment</label>";
 	} else { 		
 		$paymentStatus = "<label class='label label-warning' style='background-color:#de0707'>No Payment</label>";
 	} // /else
